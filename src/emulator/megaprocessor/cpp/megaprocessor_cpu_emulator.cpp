...
// headers (header)
// native/cpp/emulation/headers.cpp
#include <string>
#include <math.h>

#include "platforms/megaprocessor/common/cpp/memory.h"
#include "megaprocessor_cpu_emulator.h"



#include "emf/emf.h"
class megaprocessor_cpu_emulator {
public:
#include "megaprocessor_cpu_emulator.alu.h"
/*
** Pre-build elements
**
*/
// access ()
// native/cpp/emulation/access.cpp
...
void set(emfNumber<uint8_t>& r, int v) {
	r.assign(v);
}
...
uint8_t get(emfNumber<uint8_t>& r) {
	return r.getUnsigned();
}
...
uint16_t alu_add_u16u8(uint16_t v1, uint8_t v2, uint8_t v3 = 0) {
...
// alu_flags ()
// native/cpp/emulation/alu_flags.cpp
		bool wasXFlag;
		bool wasIFlag;
...
// emf_maths ()
// native/cpp/emulation/emf_maths.cpp
...
uint16_t emf_Maths_add_u16s8(uint16_t v1, uint8_t v2, uint8_t v3 = 0) {
	return alu_add_u16s8(v1, v2, v3);
}
...
// compute_flags ()
// native/cpp/emulation/compute_flags.cpp
...
// traps ()
// native/cpp/emulation/traps.cpp
...
// counters ()
// native/cpp/emulation/counters.cpp
...
// interrupts ()
// native/cpp/emulation/interrupts.cpp
...
**
*/
uint16_t read2(uint16_t addr) {
return read8(addr) & 0x03;
}
...
**
*/
uint16_t read3(uint16_t addr) {
return read8(addr) & 0x07;
}
...
**
*/
uint16_t read4(uint16_t addr) {
return read8(addr) & 0x0f;
}
...
**
*/
uint16_t read5(uint16_t addr) {
return read8(addr) & 0x1f;
}
...
**
*/
uint16_t read6(uint16_t addr) {
return read8(addr) & 0x3f;
}
...
**
*/
uint16_t read7(uint16_t addr) {
return read8(addr) & 0x7f;
}
...
**
*/
uint16_t read9(uint16_t addr) {
return read16(addr) & 0x1ff;
}
...
**
*/
uint16_t read10(uint16_t addr) {
return read16(addr) & 0x3ff;
}
...
**
*/
uint16_t read11(uint16_t addr) {
return read16(addr) & 0x7ff;
}
...
**
*/
uint16_t read12(uint16_t addr) {
return read16(addr) & 0xfff;
}
...
**
*/
uint16_t read13(uint16_t addr) {
return read16(addr) & 0x1fff;
}
...
**
*/
uint16_t read14(uint16_t addr) {
return read16(addr) & 0x3fff;
}
...
**
*/
uint16_t read15(uint16_t addr) {
return read16(addr) & 0x7fff;
}
...
*/
emfNumber<u16> r0 = 0;
emfNumber<u16> gsRegisterR0 = 0;
emfNumber<u16> r1 = 0;
emfNumber<u16> gsRegisterR1 = 0;
emfNumber<u16> r2 = 0;
emfNumber<u16> gsRegisterR2 = 0;
emfNumber<u16> r3 = 0;
emfNumber<u16> gsRegisterR3 = 0;
emfNumber<u16> pc = 0;
emfNumber<u16> gsRegisterPC = 0;
emfNumber<u16> sp = 0;
emfNumber<u16> gsRegisterSP = 0;
emfNumber<u16> ix = 0;
emfNumber<u16> gsRegisterIX = 0;
emfNumber<u16> iy = 0;
emfNumber<u16> gsRegisterIY = 0;
emfNumber<u8> ps = 0;
emfNumber<u8> gsRegisterPS = 0;
/*
** Internal state
**
*/
...
**
*/
void setRegisterValueR0(uint16_t v) {
   r0.assign(v);
}
...
**
*/
void setRegisterValueR1(uint16_t v) {
   r1.assign(v);
}
...
**
*/
void setRegisterValueR2(uint16_t v) {
   r2.assign(v);
}
...
**
*/
void setRegisterValueR3(uint16_t v) {
   r3.assign(v);
}
...
**
*/
void setRegisterValuePS(uint8_t v) {
   ps.assign(v);
}
...
**
*/
uint16_t getRegisterValue(std::string &name) {
...
**
*/
uint16_t setRegisterValue(std::string &name, uint16_t v) {
  if (name == "r0") setRegisterValueR0(v);
  if (name == "r1") setRegisterValueR1(v);
  if (name == "r2") setRegisterValueR2(v);
  if (name == "r3") setRegisterValueR3(v);
  if (name == "pc") setRegisterValuePC(v);
  if (name == "sp") setRegisterValueSP(v);
  if (name == "ix") setRegisterValueIX(v);
  if (name == "iy") setRegisterValueIY(v);
  if (name == "ps") setRegisterValuePS(v);
}
...
**
*/
bool setFlagValue(std::string &name, bool v) {
...
**
*/
bool changeFlagU(bool newState) {
// flag is uflag
if (newState) {
setFlagU();
} else {
...
pc.assign(alu_add_u16u16(emf_Maths_add_u16s8(pc,read8(pc.getUnsigned() + (1))), 0));
cycles += 1;
}
pc.add(2);
 return cycles;
...
**
*/
void getState(struct CPUState* &pState) {
pState->rr0 = getRegisterValueR0();
pState->rr1 = getRegisterValueR1();
pState->rr2 = getRegisterValueR2();
pState->rr3 = getRegisterValueR3();
pState->rpc = getRegisterValuePC();
pState->rsp = getRegisterValueSP();
pState->rix = getRegisterValueIX();
pState->riy = getRegisterValueIY();
pState->rps = getRegisterValuePS();
...
**
*/
void setState(const struct CPUState* &pState) {
setRegisterValueR0(pState->rr0);
setRegisterValueR1(pState->rr1);
setRegisterValueR2(pState->rr2);
setRegisterValueR3(pState->rr3);
setRegisterValuePC(pState->rpc);
setRegisterValueSP(pState->rsp);
setRegisterValueIX(pState->rix);
setRegisterValueIY(pState->riy);
setRegisterValuePS(pState->rps);

changeFlagU(pState->fU);
changeFlagD(pState->fD);
changeFlagC(pState->fC);
changeFlagX(pState->fX);
changeFlagV(pState->fV);
changeFlagZ(pState->fZ);
changeFlagN(pState->fN);
changeFlagI(pState->fI);
}
