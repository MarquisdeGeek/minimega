...
uint8_t alu_sub_u8u8b(uint8_t v1, uint8_t v2, uint8_t v3 = 0) {
  ...
  u16 result = (v1 - v2) - v3;
  // C version is  u16 result = ((uint16_t)v1 - (uint16_t)v2) - v3;
...
void alu_main_start() {
        ...
        for (int i = 0; i < 256; ++i) {
            parityLUT8[i] = alu_main_calculateParity(i); // we know how the C code expands 'calculateParity' to this full version
        }
...
uint8_t alu_main_calculateParity(uint8_t v, uint8_t sz = 8) {
